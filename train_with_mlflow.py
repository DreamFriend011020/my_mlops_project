...
experiment_name = "diabetes_classification"                      # 실험 묶음 이름 (MLflow UI에서 폴더처럼 표시됨)
mlflow.set_experiment(experiment_name)                       # 없으면 자동 생성, 있으면 해당 실험에 기록
 
# ── 1. 데이터 로드 및 전처리 ──────────────────────────────────
try:
    df = pd.read_csv("data/diabetes.csv")                   # DVC로 관리되는 데이터 파일 로드
    df = df.select_dtypes(include=['number']).dropna()        # 수치형 컬럼만 선택 + 결측치 행 제거
    X = df.drop('Outcome', axis=1)                            # 입력 특성 (꽃받침/꽃잎 길이·너비)
    y = df['Outcome']                                         # 정답 레이블 (0=setosa, 1=versicolor, 2=virginica)
...
registered = mlflow.register_model(
    ...
    name="diabetes_classifier"                                   # Registry에서 사용할 모델 이름
)
...
client.set_registered_model_alias(
    name="diabetes_classifier",                                  # 대상 모델 이름
